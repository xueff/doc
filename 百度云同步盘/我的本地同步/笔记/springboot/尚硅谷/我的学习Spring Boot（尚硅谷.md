...
`jdk版本修改：我的学习maven.md-- #修改maven默认JDK版本`
...
```xml
 ...
 <!-IDEA:- maven project>lifecycle>package；-->
    <build>
        <plugins>
            <plugin>
                <groupId>org.springframework.boot</groupId>
...
### 3.禁用特定自动配置类
	...
	如果发现正在应用您不需要的特定自动配置类，则可以使用exclude属性`@EnableAutoConfiguration`禁用它们 
...
如果类不在类路径上，则可以使用`excludeName`注释的属性并指定完全限定名称。最后，您还可以使用该`spring.autoconfigure.exclude`属性控制要排除的自动配置类列表 。
